...
###########################################################
model_sample = model_sample.fillna(0)
...
for kernel in ['linear','poly','rbf','sigmoid','cosine','precomputed']:
	pca = KernelPCA(n_components=20, kernel='linear', n_jobs=-1)
	pca.fit(X)
	X_pca= pca.transform(X)
	X_pred_pca = pca.transform(X_pred)
	X = np.hstack((X,X_pca))
	X_pred = np.hstack((X_pred,X_pred_pca))
...
FA.fit(X)
X_FA = FA.transform(X)
X_pred_FA = FA.transform(X_pred)
X = np.hstack((X,X_FA))
X_pred = np.hstack((X_pred,X_pred_FA))
'''

# FastICA(n_components=None)
